...
// A nutrition label, keyed by unit name (e.g. 'cup', 'tsp'). The recipe's unit
// must match a key here or the ingredient is flagged "can't compute" — there's
// no unit conversion. Values are per serving; servingSize is in the keyed unit.
...
// A specific purchasable SKU. This is the single place brand identity lives:
// it carries BOTH the nutrition label (brand-specific) and the purchase info
// (where to buy, price, package size). An ingredient lists the products it can
// resolve to; exactly one is selected (see Ingredient.defaultBrand) and that
// one's label drives nutrition. Nutrition is never blended across brands.
export interface Product {
    brand: string;          // identity, e.g. 'Milkadamia', '365', 'Navitas'
    variant?: string;       // 'Unsweetened', 'Creamy', 'Crunchy', 'Organic'
    store?: string;         // where to buy, e.g. 'Whole Foods', 'Amazon'
    link?: string;
    price?: number;         // package price
    size?: number;          // package size
    sizeUnit?: Unit;        // package unit (ounce, pound, ...)
    organic?: boolean;
...
export interface Ingredient {
    ...
    name: string;           // generic display name — what shows in steps & shopping
    quantity?: number;
    unit?: Unit;
    perishableDays?: number;
    // Generic, brand-independent nutrition (use for produce: celery, banana…).
    // For packaged goods that vary by brand, put nutrition on a Product instead.
    nutrition?: NutritionLabel;
    // Candidate brands/SKUs this ingredient can resolve to (purchase + nutrition).
    products?: Product[];
    // Which product is selected — its label drives nutrition and it's marked as
    // the pick on the shopping list. Matches Product.brand; falls back to the
    // first product if unset, then to ingredient-level `nutrition`.
    defaultBrand?: string;
    ...
    // should reflect that in their display text.
...
export interface Step {
    ...
    recipeId?: string;
    // Timer step IDs that must complete before this step is interactable.
    // Populated automatically by buildCookingPlan for equipment/ingredient
    // conflicts, or manually via waitFor() for culinary constraints that
    // can't be derived from code logic (e.g. "don't dip until panko is ready").
    waitForIds?: number[];
    // Fluent method to manually declare a dependency on another step.
    // Returns this step so it can be chained inline.
...
export interface Recipe {
    ...
    // True for synthetic recipes created via "+ Add an ingredient" — a one-off
    // ingredient wrapped as a single-step recipe so it flows through the
    // shopping/nutrition pipelines without parallel codepaths. Cooking skips
    // these so they don't introduce meaningless steps into the cook flow.
    adhoc?: boolean;
}
    ...
    buildShoppingList().forEach((ingredient) => {
        ...
        if (products.length > 0) {
            ...
            products.forEach((product) => {
                ...
                if (product === chosen) a.classList.add('default-product');
...
let readySection:   HTMLElement | null = null;
let waitingSection: HTMLElement | null = null;

function getOrCreateCookingSections(): { ready: HTMLElement; waiting: HTMLElement } {
...
function promotePanelToReady(panel: HTMLElement): void {
    ...
    const panels = Array.from(readySection.querySelectorAll<HTMLElement>(':scope > .panel[data-step-index]'));
    const insertBefore = panels.find(p => {
        if (p.classList.contains('completed') || p.classList.contains('skipped')) return false;
