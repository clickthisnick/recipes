...
class BakedZiti extends Recipe {
    constructor() {
        ...
        this.steps = [
            // '(1.5 quarts)
            e.pot().add([i.water(6, u.cup)]),
            e.pot().boilWithLid(10, 'm', 7),
            text.set(['mince', i.garlicClove(4, u.unit)]),
            text.set(['mince', i.whiteOnion(1, u.unit)]),
            Timer.end(),
            e.pot().add([i.ziti(16, u.ounce), i.oliveOil(1, u.tbsp)]),
            e.pot().cookWithLid(13, 'm', 7),
            e.pan().preheat(7),
            Timer.end(),
            text.set(['Preheat oven to 350']),
            e.pan().add([i.italianSausage(16, u.ounce)]),
            e.pan().cook(5, 'm'),
            Timer.end(),
            e.pan().add([i.garlicClove(), i.Groups.onion(), i.italianSeasoning(1, u.tsp)]),
            e.pan().cook(5, 'm'),
            Timer.end(),
            e.pan().add([i.spaghettiSauce(16, u.ounce)]),
            e.pan().cook(1, 'm'),
            Timer.end(),
            Timer.end(),
            ...
            text.set([
                'Pour the rest of the sauce over the pasta, dot the remaining',
                i.ricottaCheese(7, u.ounce),
                'over the pasta, and sprinkle',
                i.mozzarellaCheese(8, u.ounce),
                i.parmesanCheese(3, u.ounce),
                'on top.',
            ]),
