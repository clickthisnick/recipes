...
export class MealRecipe extends RecipeContainer {
    constructor() {
        super();
        this.variations = [Biryani1PoundChicken1CupRice, Biryani2PoundChicken2CupRice];
    }
}
...
class Biryani1PoundChicken1CupRice extends Recipe {
    constructor() {
        ...
        this.steps = [
            e.bowl(0).add([
                i.plainYogurt(0.5, u.cup),
                i.gramMasala(1, u.tsp),
                i.chilliPowder(1, u.tsp),
                i.salt(1, u.tsp),
                i.dhanaJeera(1, u.tbsp), // i.cilantro(2, u.tbsp),
                i.gingerPowder(0.5, u.tsp), // i.gingerPaste(.5, u.tsp),
                i.garlicPowder(0.5, u.tsp), // i.garlicPaste(.5, u.tsp),
                i.turmeric(0.25, u.tsp),
                i.lemonJuice(1, u.tbsp),
                i.mintLeaf(2, u.tbsp),
                i.chickenBreast(1, u.pound), // i.chickenBreast(1, u.pound),
            ]),
            Timer.set(30, 'm', 'Let chicken marinade'),
            i.yellowOnion(0.5, u.pound).cutIntoThinSlices(),
            Timer.end(),
            e.instantPot().add([i.butter(2, u.tbsp), i.yellowOnion()]),
            e.instantPot().sautee(10, 'm'),
            e.bowl(1).add([i.coconutMilk(2, u.tbsp), i.saffron(10, u.stick)]),
            e.bowl(1).microwave(30, 's'),
            Timer.end(),
            Timer.end(),
            text.set(['Take out half of the onions and put into a bowl']),
            text.set([
                'Put onions to one side of the instapot, so you can add the whole spices to the other side',
            ]),
            e.instantPot().add([
                // 1 stick Cinnamon (Dalchini)
                i.bayLeaf(2, u.unit),
                i.greenCardamonSeed(0.5, u.tsp),
                i.wholeBlackPepper(0.5, u.tsp),
                i.clove(4, u.unit),
            ...
            e.instantPot().add([i.chickenBreast()]),
            Timer.set(10, 'm', 'Cook chicken'),
            Timer.end(),
            text.set([
                'In the next step, when you add the rice on top of the chicken, make sure to flatten it out.',
            ]),
            e
                .instantPot()
                .add([
...
class Biryani2PoundChicken2CupRice extends Recipe {
    constructor() {
        ...
        this.steps = [
            ...
            Timer.end(),
            e.instantPot().add([i.butter(1, u.tbsp), i.yellowOnion()]),
            e.instantPot().sautee(12, 'm'),
            e.bowl(1).add([i.coconutMilk(4, u.tbsp), i.saffron(20, u.stick)]),
            e.bowl(1).microwave(15, 's'),
            Timer.end(),
            Timer.end(),
            text.set(['Take out half of the onions and put into a bowl']),
            text.set([
