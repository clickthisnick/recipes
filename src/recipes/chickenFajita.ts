...
export class ChickenFajita extends Recipe {
    constructor() {
        ...
        this.steps = [
            ...
            Timer.end(),
            e.pan().add([i.coconutOil(1, u.tbsp), i.ingredient('Marinaded Chicken')]),
            e.pan().cookWithLidSlightlyOff(5, 'm', 4),
            text.set(['Flip Chicken']),
            Timer.end(),
            e.pan().cookWithLidSlightlyOff(5, 'm', 4),
            i.Groups.onion(0.5, u.unit).cutIntoStrips(),
            i.Groups.mushroom(4, u.ounce).cutIntoStrips(),
            i.greenBellPepper(1, u.unit).cutIntoStrips(),
            Timer.end(),
            text.set(['Take chicken out of pan']),
            e.pan().add([i.Groups.onion(), i.Groups.mushroom(), i.greenBellPepper()]),
            e.pan().cookWithLidSlightlyOff(4, 'm', 4),
            Timer.end(),
            e.pan().add([i.ingredient('Cooked Chicken'), i.limeJuice(2, u.tsp)]),
            e.pan().cookWithLidSlightlyOff(1, 'm', 2),
            e.pan().stir(),
            Timer.end(),
            text.set([
                'Add ingredients to',
                i.softTortillaShell(1, u.unit),
                'with',
                i.Groups.cheese(1, u.unit),
                'with',
                i.salsa(1, u.unit),
            ]),
        ];
