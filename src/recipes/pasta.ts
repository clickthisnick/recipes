...
export class MealRecipe extends RecipeContainer {
    constructor() {
        ...
        this.variations = [
            LentilPenne,
            LentilSpaghetti,
            LentilSpaghettiInstantPot,
            DontUseModernBrandLentilPenne,
        ];
    }
}
...
export class LentilPenne extends Recipe {
    constructor() {
        super();
        this.steps = [
            e.pot().add([i.water(18, u.second)]),
            Timer.set(10, 'm', 'Wait for water to boil'),
            i.Groups.mushroom(4, u.unit).cutIntoStrips(),
            e.pan().add([i.oliveOil(1, u.tsp), i.Groups.mushroom(4, u.unit)]),
            e.pan().cook(8, 'm'),
            Timer.end(),
            Timer.end(),
            e.pot().add([i.lentilSpaghetti(8, u.ounce)]),
            Timer.set(10, 'm', 'let lentil penne cook'),
            e.pan().add([i.spaghettiSauce(25, u.ounce), i.oliveOil(1, u.tsp)]),
            e.pan().cook(6, 'm'),
            Timer.end(),
            Timer.end(),
...
export class LentilSpaghetti extends Recipe {
    constructor() {
        super();
        this.steps = [
            e.pot().add([i.water(25, u.second)]),
            Timer.set(15, 'm', 'Wait for water to boil'),
            i.Groups.mushroom(4, u.unit).cutIntoStrips(),
            Timer.end(),
            e.pot().add([i.lentilSpaghetti(8, u.ounce)]),
            Timer.set(18, 'm', 'let lentil spaghetti cook'),
            e.pan().add([i.Groups.mushroom(4, u.unit)]),
            e.pan().cook(8, 'm'),
            e.pan().add([i.spaghettiSauce(25, u.ounce)]),
            e.pan().cook(8, 'm'),
            Timer.end(),
            Timer.end(),
            Timer.end(),
