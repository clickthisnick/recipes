...
export class Serializer {
    ...
    public static convertUnitIntoStr(unit: string, quantity: number): string {
        ...
        let unitString = quantity > 1 ? differentSpellingMap[unit] || `${unit}s` : unit;
            ...
            if (stepz.hasOwnProperty('children')) {
                stepz.children.forEach((child) => {
                    if (child.text.startsWith(Serializer.lazyIngredientIdx)) {
                        const idx = parseInt(
                            child.text.substring(Serializer.lazyIngredientIdx.length + 1)
                        );
                        child.text = [
                            '•',
                            Serializer.turnIngObjIntoStr(child.ingredients[idx], true),
                        ].join(' ');
                    }
                });
            }
