...
export class MealRecipe extends RecipeContainer {
    constructor() {
        super();
        this.variations = [ThaiBasil, ThaiBasil2Servings, ThaiBasilNoFreshIngredients];
    }
}
...
class ThaiBasil2Servings extends Recipe {
    constructor() {
        super();
        this.steps = [
            e.instantPot().add([i.whiteRice(1.5, u.cup), i.water(2.5, u.cup)]),
            ...
            e.pan().add([i.peanutOil(2, u.tbsp)]),
            e.pan().preheat(7),
            e.plate().add([i.garlicClove(), i.serranoChilli(), i.greenOnion()]),
            i.chickenBreast(1, u.pound).dice(),
            Timer.end(),
            e.pan().add([i.garlicClove(), i.serranoChilli()]),
            Timer.set(10, 's'),
            Timer.end(),
            e.pan().add([i.greenOnion(), i.chickenBreast()]),
            Timer.set(4, 'm'),
            Timer.end(),
            e.pan().add([i.ingredient('sauce')]),
            Timer.set(2, 'm'),
            Timer.end(),
            e.instantPot().pressureRelease(4, 'm'),
            Timer.end(),
            e.pan().add([i.basil(2, u.cup)]),
        ];
    }
}
...
class ThaiBasil extends Recipe {
    constructor() {
        super();
        this.steps = [
            e.instantPot().add([i.whiteRice(0.75, u.cup), i.water(1.25, u.cup)]),
            ...
            e.plate().add([i.garlicClove(), i.serranoChilli(), i.greenOnion()]),
            e.pan().add([i.peanutOil(1.5, u.tbsp)]),
            e.pan().preheat(7),
            i.chickenBreast(0.5, u.pound).dice(),
            Timer.end(),
            e.pan().add([i.garlicClove(), i.serranoChilli()]),
            Timer.set(10, 's'),
            Timer.end(),
