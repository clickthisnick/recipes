...
export class MealRecipe extends RecipeContainer {
    constructor() {
        super();
        this.variations = [BlueprintCurry];
    }
}
...
class BlueprintCurry extends Recipe {
    constructor() {
        super();
        this.steps = [
            i.broccoli(0.5, u.cup).cutIntoOneInchPieces(),
            i.cauliflower(0.5, u.cup).cutIntoOneInchPieces(),
            // Good to wait up to 40 min for anti cancing properties
            i.whiteOnion(1, u.unit).dice(),
            text.set(['Turn pot to medium 5 heat']),
            e.pot().add([i.avocadoOil(0.5, u.tbsp)]),
            ...
            e.pot().add([i.carrots(), i.blackPepper(5, u.crack), i.turmeric(1, u.tsp)]),
            e.pot().stir(),
            e.pot().add([i.broccoli(), i.cauliflower(), i.water(2, u.cup)]),
            e.pot().stir(),
            text.set(['Bring heat of pot up to a simmer']),
            e.pot().cookWithLid(15, 'm', 7),
            e.pot(2).add([i.vegetableBroth(2, u.cup), i.quinoa(1, u.cup)]),
            text.set(['Bring heat of pot up to a boil']),
            e.pot(2).cook(15, 'm', 7),
            i.lime(2, u.unit).juice(),
            i.greenBeans(0.5, u.cup).cutInHalf(),
            i.cilantro(0.75, u.cup).cube(),
            Timer.end(),
            Timer.end(),
