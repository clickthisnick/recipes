...
export class MealRecipe extends RecipeContainer {
    constructor() {
        super();
        this.variations = [SitrFryChicken, StirFryVegan];
    }
}
...
class SitrFryChicken extends Recipe {
    constructor() {
        ...
        this.steps = [
            ...
            e.pan().add([i.Groups.onion(), i.carrots(), i.celery(), i.whiteMushroom()]),
            e.pan().cook(3, 'm', 4),
            Timer.end(),
        ];
    }
...
class StirFryVegan extends Recipe {
    constructor() {
        ...
        this.steps = [
            ...
            Timer.end(),
            e.pan().add([i.oliveOil(1, u.tbsp), i.carrots()]),
            Timer.set(2, 'm'),
            text.set(['Cut', i.yellowOnion(2, u.unit)]),
            Timer.end(),
            e.pan().add([i.yellowOnion()]),
            Timer.set(2, 'm'),
            text.set(['Cut', i.broccoli(2, u.unit)]),
            Timer.end(),
            e.pan().add([i.broccoli()]),
            Timer.set(1, 'm'),
            text.set(['Cut', i.whiteMushroom(5, u.unit)]),
            Timer.end(),
            e.pan().add([i.whiteMushroom()]),
            Timer.set(4, 'm'),
