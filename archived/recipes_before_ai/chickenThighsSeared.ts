...
class ChickenThighsSeared extends Recipe {
    constructor() {
        ...
        this.steps = [
            ...
            i.chickenThigh().seasonWith([i.ingredient('Bowl season mixture')]),
            Timer.end(),
            text.set(['Make sure thighs are cooked smooth side down first']),
            e.pan().add([i.chickenThigh()]),
            e.pan().cook(5, 'm', 6),
            Timer.end(),
            ...
            Timer.end(),
            e.pan().add([i.ingredient('Garlic slices')]),
            e.pan().cook(0.5, 'm', 6),
            Timer.end(),
            e.pan().add([i.chickenStock(0.5, u.cup)]),
