...
const state = {
    ...
    selectedRecipeIds: [],
    timers: new Map(),
    ringingAlarms: new Map(),
    gateRunCounts: new Map(),
    screen: 'select',
    searchQuery: '',
    cookingPlanSteps: [],
...
export const e = {
    ...
    microwave: (label) => new Equipment('microwave', label),
    ...
    siliconeBag: (label) => new Equipment('silicone bag', label),
};
// ============================================================
// RECIPE REGISTRATION
// ============================================================
...
function inventoryKey(ingredient, byPackage) {
...
function unitsPerPackage(ing, product) {
    const recipeUnit = ing.unit?.name;
    if (!recipeUnit || product.size === undefined || !product.sizeUnit)
        return null;
    const packageUnit = product.sizeUnit.name;
    if (recipeUnit === packageUnit)
        return product.size;
    const packageUnitsPerRecipeUnit = convertWithDensity(1, recipeUnit, packageUnit, ing.conversions);
    if (!packageUnitsPerRecipeUnit)
        return null;
...
function renderSelectScreen() {
    ...
    if (hasHiddenRecipes) {
        const toggleBtn = createButton(state.showHidden ? 'Hide hidden recipes' : 'Show hidden recipes', () => {
...
function renderShoppingScreen() {
    ...
    buildShoppingList().forEach((ingredient) => {
        ...
        if (trackByPackage) {
            ...
            hint.textContent = `1 ${packageWord(1)} ≈ ${formatQuantityUnit(perPackage, ingredient.unit)}${product?.variant ? ` (${product.variant})` : ''}`;
            invRow.appendChild(hint);
        }
        const updateNeeded = (have) => {
            const haveInRecipeUnits = trackByPackage ? have * perPackage : have;
            ...
            if (needed <= 0 && required > 0) {
                ...
                if (trackByPackage) {
                    const packagesNeeded = Math.ceil(needed / perPackage);
                    if (packagesNeeded > 0)
                        text += ` (~${packagesNeeded} more ${packageWord(packagesNeeded)})`;
                }
...
function renderNutritionScreen() {
    ...
    perRecipeEff.forEach(({ recipe, servings, nutrition, excluded, effTotals, effCost }) => {
        ...
        if (nutritionRecipeExclusions.has(recipe.id))
            section.classList.add('nutrition-recipe-excluded');
        else
        ...
        eatenBtn.addEventListener('click', (ev) => {
            ev.stopPropagation();
            if (isRecipeExcluded)
                nutritionRecipeExclusions.delete(recipe.id);
            else
                nutritionRecipeExclusions.add(recipe.id);
            renderNutritionScreen();
        });
...
function renderCookingScreen() {
    ...
    for (let idx = 0; idx < state.cookingPlanSteps.length; idx++) {
        ...
        if (step.type === 'info') {
            const group = [step];
            while (idx + 1 < state.cookingPlanSteps.length &&
...
function renderInfoGroup(steps) {
    const panel = document.createElement('div');
...
function formatOverdueDuration(totalSeconds) {
...
function startAlarm(step, panel, dueAt, onAcknowledge) {
...
function stopAlarm(stepId) {
    const intervalId = state.ringingAlarms.get(stepId);
    if (intervalId !== undefined) {
        window.clearInterval(intervalId);
        state.ringingAlarms.delete(stepId);
    }
...
function acknowledgeAlarm(stepId) {
...
const BUILD_TIME = '2026-08-23T13:07:42.865Z';
...
function appendBuildFooter(root) {
...
registerRecipe(createRecipe('dill-dip', 'Dill Dip', (() => {
    const bowl = e.bowl();
...
registerRecipe(withPlan(createRecipe('taco-ames-hot-sauce', 'Taco Ames Hot Sauce', (() => {
    const cuttingBoard = e.cuttingBoard();
    const pot = e.pot();
    const mixer = e.bulletMixer();
    ...
    return steps;
})(), 'Sauces'), { planMinutes: 1, portable: true, prepMinutes: 20, perishableDays: 14 }));
...
registerRecipe(withPlan(createRecipe('test-recipe-all-features', 'Test Recipe (All Features)', (() => {
    ...
    return steps;
})(), 'Testing', 2), { planMinutes: 2, portable: false, prepMinutes: 1, perishableDays: 1, hidden: true }));
// ============================================================
// BUNDLES
// ============================================================
const DAILY_BLUEPRINT_RECIPE_IDS = [
    'blueprint-smoothie',
